...
test("computeAgreedTotalThisMonth sums only completed jobs in the current month", () => {
  const now = new Date("2026-08-23T12:00:00.000Z");
  const jobs = [
    makeJob({ id: 1, created_at: "2026-08-20T12:00:00.000Z", status: "completed", agreed_price: "200" }),
    makeJob({ id: 2, created_at: "2026-08-10T12:00:00.000Z", status: "completed", agreed_price: "150" }),
    // Different month — excluded.
    makeJob({ id: 3, created_at: "2026-07-15T12:00:00.000Z", status: "completed", agreed_price: "999" }),
    // Not completed — excluded even though it has an agreed_price.
    makeJob({
      id: 4,
  ...
  expect(computeAgreedTotalThisMonth(jobs, now)).toBe(350);
});
